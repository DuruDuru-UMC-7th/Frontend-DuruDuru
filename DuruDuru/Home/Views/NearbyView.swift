...
class NearbyView: UIView {
    ...
    private var trades: [HomeTradeModel] = []
    ...
    // API 데이터를 반영하는 함수 추가
    func updateTradeList(_ newTrades: [HomeTradeModel]) {
        self.trades = newTrades
        reloadTradeListView()
    }
    
    // 품앗이 리스트 UI 업데이트
    private func reloadTradeListView() {
        listStackView.arrangedSubviews.forEach { $0.removeFromSuperview() } // 기존 UI 제거
        
        for trade in trades {
            let itemView = createTradeItemView(trade: trade)
            listStackView.addArrangedSubview(itemView)
        }
    }

    // 품앗이 개별 아이템을 생성하는 함수 (API 반영)
    private func createTradeItemView(trade: HomeTradeModel) -> UIView {
        let containerView = UIView()
        
        let imageView = UIImageView()
        if let url = URL(string: trade.thumbnailImgURL ?? "") {
        ...
        subtitleLabel.text = "\(trade.eupmyeondong) · \(trade.createdAt)"
        subtitleLabel.font = .systemFont(ofSize: 10)
        subtitleLabel.textColor = .gray
        
        let typeLabel = UILabel()
        typeLabel.text = trade.tradeType == "SHARE" ? "나눔" : "교환"
        typeLabel.font = .boldSystemFont(ofSize: 14)
        typeLabel.textColor = .black
        
        let quantityLabel = UILabel()
        quantityLabel.text = "수량 \(trade.ingredientCount)개"
